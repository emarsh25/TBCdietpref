...
#  	theme(text = element_text(size=plot_text_size), axis.title = element_blank(), legend.position = "none", axis.line = element_line(size = 0.2), axis.text = element_text(angle = 45), axis.ticks = element_blank()) +
  ...
  ## function to calculate stats for # of sips
calc_sip_stats <- function(gh_filepath,j=3) {
GET(gh_filepath, write_disk(tf <- tempfile(fileext = ".xls")))
...
out_df <- data.frame(trt=character(), pref=numeric(),day=numeric(),time=numeric(),sip1 = numeric(), sip2 = numeric(), dieta=character(),dietb=character())
  ...
  pd3e$abc2.x <- plyr::revalue(pd3e$tt, c("Ax.sipsA" = "Ax control","At.sipsA" = "At control", "Lb.sipsA" = "Lb control","Ax.sipsB" = "Ax trial", "At.sipsB" = "At trial", "Lb.sipsB" = "Lb trial"))
  pd3e$abc2.x <- factor(pd3e$abc2.x, levels = c("Ax control","Ax trial","At control", "At trial", "Lb control", "Lb trial"))
...
#  	theme(text = element_text(size=plot_text_size), axis.title = element_blank(), legend.position = "none", axis.line = element_line(size = 0.2), axis.text = element_text(angle = 45), axis.ticks = element_blank()) +
  	...
  	geom_errorbar(col = "black", aes(ymax =mean+sem, ymin = mean-sem), width=0.25, size=.25) +
  #	geom_hline(yintercept = 0.5, linetype="dashed") +
 	geom_text(aes(label=c(diff_codes[c(3,1,5,4,2,6)]), y=mean + sem + 40), size=plot_text_size/2)
  return(p25g100y)
}

calc_sip_stats_axenic <- function(gh_filepath,j=3, fileadd ="", append_val=F) {
GET(gh_filepath, write_disk(tf <- tempfile(fileext = ".xls")))
...
out_df <- data.frame(trt=character(), pref=numeric(),day=numeric(),time=numeric(),sip1 = numeric(), sip2 = numeric(), dieta=character(),dietb=character())
  ...
  write.table(data.frame(efg) %>% 
    tidyr::separate(col = comparisons, into = c("comp1","comp2"), sep = " - ", remove = T) %>%
    rowwise() %>%
    filter((grepl("sipsA", comp1) & grepl("sipsA",comp2)) | (grepl("sipsB", comp1) & grepl("sipsB",comp2))) %>%
    filter(grepl("Ax",comp1) | grepl("Ax",comp2)) %>%
    mutate(diet = fileadd), "Figs2c_data.csv", append = append_val, sep = ",", row.names = F, quote = F)
}

calc_sip_stats_axenic_plots <- function(gh_filepath,j=3, sigvals = "a") {
GET(gh_filepath, write_disk(tf <- tempfile(fileext = ".xls")))
...
#  	theme(text = element_text(size=plot_text_size), axis.title = element_blank(), legend.position = "none", axis.line = element_line(size = 0.2), axis.text = element_text(angle = 45), axis.ticks = element_blank()) +
  	...
  	geom_errorbar(col = "black", aes(ymax =mean+sem, ymin = mean-sem), width=0.25, size=.25) +
  #	geom_hline(yintercept = 0.5, linetype="dashed") +
 	geom_text(aes(label=c(sigvals), y=mean + sem + 40), size=plot_text_size/2)
  return(p25g100y)
}
...
#### plot 100g100y
```{r fig1100g100y, exercise = T, warning = F, message = F, eval = T, exercise.timelimit = 10000}
...
#### 100g50y (adapted to become Figure 1)
...
#### plot 100g100y
```{r fig1100g100y2, exercise=T, warning = F, message = F, eval = T, exercise.timelimit = 10000}
...
#### Figure code
```{r figs2b, exercise=T, warning = F, message = F, eval = T, exercise.timelimit = 10000}
...
### Figure S2C sip distribution {.tabset}
#### Figure code
```{r figs2c, exercise = T, warning = F, message = F, eval = T, exercise.timelimit = 10000}
...
#### Build the plot
```{r, exercise=F, warning = F, message = F, eval = F, exercise.timelimit = 10000}
...
### Figure S2C sip distribution - against axenic {.tabset}
#### Figure code
```{r figs2cpart2, exercise=T, warning = F, message = F, eval = T, exercise.timelimit = 10000}
...
```{r FigS3C, exercise=T, warning = F, message = F, eval = T, exercise.timelimit = 1000000,fig.height=10, fig.width=3.5}
...
```{r figs5c, exercise=T, warning = F, message = F, eval = T, fig.width = 5, exercise.timelimit = 10000}
